...
def test_get_remi():
    """Test the get of a trainer."""
    response = client.get("trainers/1")
    assert response.status_code == 200
    assert response.json() == {"name": "Rémi",
    "birthdate": "2002-06-20",
    "id":1,
    "inventory": [
        {'description': 'Permet de soigner un pokemon à la hauteur de 50% de ses PV',
        'id': 1,
        'name': 'Potion de vie',
        'trainer_id': 1}],
        'pokemons': [
...
def test_random_is_three():
    """Test the random pokemon."""
    response = client.get("/pokemons/random/")
    assert response.status_code == 200
    assert len(response.json()) == 3

def test_get_items():
    """Test the get of items."""
    response = client.get("/items")
    assert response.status_code == 200
    assert response.json()[0] == {"name": "Potion de vie",
    "description": "Permet de soigner un pokemon à la hauteur de 50% de ses PV",
    "id": 0,
    "trainer_id": 0}

def test_get_pokemons():
    """Test the get of pokemons."""
    response = client.get("/pokemons")
    assert response.status_code == 200
    assert response.json()[0] == {"api_id": 1,
    "custom_name": "Balba",
    "id": 0,
    "name": "Balbusaur",
    "trainer_id": 0}
